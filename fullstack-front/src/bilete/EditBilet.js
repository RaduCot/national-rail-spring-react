...
export default function EditBilet() {
    ...
    const onSubmit = async (e) => {
        e.preventDefault();
        await axios.put(`http://localhost:8080/bilet/${id}`, bilet);
        navigate("/tickets");
    }

    const loadBilet = async () => {
        const result = await axios.get(`http://localhost:8080/bilet/${id}`);
        setBilet(result.data);
    }
